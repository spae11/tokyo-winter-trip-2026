...
s=p.read_text()
marker='  /* TRAVEL_BUY_RULES_V66 — extends Plan Extras; no separate module. */'
if 'SOUVENIR_GUIDE_V80' not in s:
    block=r'''
...
'''
    if marker not in s:
        raise SystemExit('souvenir insertion marker missing')
    s=s.replace(marker,block+'\n'+marker,1)
    p.write_text(s)

sw=Path('sw.js')
t=sw.read_text().replace("const CACHE='our-journey-v79';","const CACHE='our-journey-v80';",1)
if t==sw.read_text() and "our-journey-v80" not in t:
    raise SystemExit('cache version marker missing')
sw.write_text(t)

idx=Path('index.html')
x=idx.read_text()
x=x.replace('manifest.webmanifest?v=79','manifest.webmanifest?v=80')
x=x.replace('ourJourneySWReloadV79','ourJourneySWReloadV80')
x=x.replace('./sw.js?v=79','./sw.js?v=80')
idx.write_text(x)

mf=Path('manifest.webmanifest')
if mf.exists():
    m=mf.read_text().replace('app=79','app=80').replace('app=76','app=80')
    mf.write_text(m)

print('souvenir guide patch complete')
