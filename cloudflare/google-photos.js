...
async function aesKey(env){const raw=await crypto.subtle.digest('SHA-256',enc.encode('google-photos:'+env.TRAVEL_SYNC_KEY));return crypto.subtle.importKey('raw',raw,{name:'AES-GCM'},false,['encrypt','decrypt'])}
...
export async function handleGooglePublic(req,env,url,origin){
  ...
  if(!configured(env))return new Response('Google Photos is not configured.',{status:503});
  await ensureTables(env);
...
export async function handleGoogleAuthed(req,env,url,origin,roomId){
  ...
  if(url.pathname==='/api/google/status'&&req.method==='GET'){
    const auth=await env.DB.prepare('SELECT updated_at FROM google_photo_auth WHERE room_id=?').bind(roomId).first();
    const count=await env.DB.prepare('SELECT COUNT(*) AS n FROM google_photo_items WHERE room_id=?').bind(roomId).first();
    return json({ok:true,configured:configured(env),connected:Boolean(auth),backupCount:Number(count?.n)||0,updatedAt:auth?.updated_at||null},200,origin);
  }
  ...
  if(url.pathname==='/api/google/items'&&req.method==='GET'){
    const rows=await env.DB.prepare('SELECT local_id,google_media_id,product_url,filename,mime_type,created_at FROM google_photo_items WHERE room_id=? ORDER BY created_at DESC LIMIT 500').bind(roomId).all();
    return json({ok:true,items:rows.results||[]},200,origin);
  }

  if(url.pathname==='/api/google/upload'&&req.method==='POST'){
    if(!env.MEDIA)return json({ok:false,error:'r2_unavailable'},503,origin);
    const localId=(url.searchParams.get('localId')||'').slice(0,180),filename=(url.searchParams.get('filename')||'travel-photo').slice(0,180),description=(url.searchParams.get('description')||'').slice(0,1000);
    if(!localId)return json({ok:false,error:'local_id_required'},400,origin);
    const existing=await env.DB.prepare('SELECT google_media_id,product_url FROM google_photo_items WHERE room_id=? AND local_id=?').bind(roomId,localId).first();
    if(existing)return json({ok:true,existing:true,mediaItemId:existing.google_media_id,productUrl:existing.product_url||null},200,origin);
    ...
    const size=Number(req.headers.get('Content-Length')||0);if(size>50000000)return json({ok:false,error:'media_too_large'},413,origin);
    ...
    const uploadToken=await up.text();if(!up.ok||!uploadToken)throw Object.assign(new Error('google_upload_failed'),{status:502});
    ...
    await env.DB.prepare(`INSERT INTO google_photo_items(room_id,local_id,google_media_id,product_url,filename,mime_type) VALUES(?,?,?,?,?,?)`).bind(roomId,localId,media.id,media.productUrl||'',filename,mime).run();
    return json({ok:true,mediaItemId:media.id,productUrl:media.productUrl||null},201,origin);
  }

  if(url.pathname==='/api/google/picker/session'&&req.method==='POST'){
    const access=await tokenForRoom(env,roomId);
    const data=await googleJson('https://photospicker.googleapis.com/v1/sessions',access,{method:'POST',body:{}});
    return json({ok:true,...data},200,origin);
  }
  const sm=url.pathname.match(/^\/api\/google\/picker\/session\/([^/]+)$/);
  if(sm&&req.method==='GET'){
    const access=await tokenForRoom(env,roomId);
    const data=await googleJson('https://photospicker.googleapis.com/v1/sessions/'+encodeURIComponent(sm[1]),access);
    return json({ok:true,...data},200,origin);
  }
  if(url.pathname==='/api/google/picker/items'&&req.method==='GET'){
    const sessionId=url.searchParams.get('sessionId')||'';if(!sessionId)return json({ok:false,error:'session_id_required'},400,origin);
    const access=await tokenForRoom(env,roomId);
    const data=await googleJson('https://photospicker.googleapis.com/v1/mediaItems?sessionId='+encodeURIComponent(sessionId)+'&pageSize=100',access);
    return json({ok:true,...data},200,origin);
  }
