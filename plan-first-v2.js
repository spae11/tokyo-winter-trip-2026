...
const path=location.pathname,isRoot=path===BASE||path===BASE+'index.html',isPlan=path.startsWith(BASE+'tokyo/')||path.startsWith(BASE+'hongkong/')||path.startsWith(BASE+'danang/');if(!isRoot&&!isPlan)return;
document.body.classList.add('pfx-v2');
if(isPlan&&!document.querySelector('script[data-plan-photo-memory]')){const s=document.createElement('script');s.src=BASE+'plan-photo-memory-v1.js?v=57';s.async=true;s.dataset.planPhotoMemory='1';document.head.appendChild(s)}
if(isRoot&&!document.querySelector('script[data-memory-delete-everywhere]')){const s=document.createElement('script');s.src=BASE+'memory-delete-everywhere-v1.js?v=57';s.async=true;s.dataset.memoryDeleteEverywhere='1';document.head.appendChild(s)}
...
async function fadeNodes(nodes,dir='in'){
  ...
  await Promise.all(els.map(el=>{
    ...
    const a=el.animate(frames,{duration:incoming?260:175,easing:incoming?'cubic-bezier(.22,1,.36,1)':'ease-in',fill:'both'});
    return a.finished.catch(()=>{}).finally(()=>{try{a.cancel()}catch{}})
  }))
}
function pageFadeTo(href){if(reduced()){location.href=href;return}document.body.classList.add('pfx-page-leaving');setTimeout(()=>location.href=href,185)}
