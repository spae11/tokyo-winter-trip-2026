...
if "id='travel-buy-rules'" not in s and 'id="travel-buy-rules"' not in s:
    marker="  injectTravelApps();\n"
    if marker not in s:
        raise SystemExit('travel apps marker not found')
    block=r'''
...
p=Path('sw.js')
s=p.read_text(encoding='utf-8')
s=re.sub(r"const CACHE='our-journey-v\d+'","const CACHE='our-journey-v66'",s,count=1)
s=s.replace('?v=65','?v=66')
s=s.replace('a[href="./tokyo/"],a[href="./hongkong/"]','a[href="./tokyo/"],a[href="./hongkong/"],a[href="./danang/"]')
p.write_text(s,encoding='utf-8')

p=Path('index.html')
s=p.read_text(encoding='utf-8')
s=re.sub(r"sw\.js\?v=\d+","sw.js?v=66",s)
p.write_text(s,encoding='utf-8')
