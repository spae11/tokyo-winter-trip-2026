...
window.OUR_JOURNEY_APP_VERSION='v92';
const $=(s,r=document)=>r.querySelector(s);
const reduce=()=>window.matchMedia?.('(prefers-reduced-motion: reduce)').matches;

// Self-heal installed PWA clients that are still controlled by an older service worker.
if('serviceWorker' in navigator){
 const reloadKey='ourJourneyAppUpgradeV92';
 navigator.serviceWorker.addEventListener('controllerchange',()=>{
  if(sessionStorage.getItem(reloadKey))return;
  sessionStorage.setItem(reloadKey,'1');
  location.reload();
 });
 navigator.serviceWorker.register('/tokyo-winter-trip-2026/sw.js?v=92',{updateViaCache:'none'}).then(async r=>{
...
load('shanghai-register-v1','__shanghaiRegisterV1','5');
